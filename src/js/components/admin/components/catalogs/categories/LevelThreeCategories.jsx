...
export const LevelThreeCategories = ({ match: { params } }) => {
  ...
  const apiCalls = {
    getCategoriesApi: categoryId => {
      ...
      categoryServices
        .getLevelThreeCategories(categoryId)
        .then(response => {
    ...
    addCategoryApi: category => {
      ...
      categoryServices
        ...
        .then(() => {
          const { levelTwoCategoryId } = params
          apiCalls.getCategoriesApi(levelTwoCategoryId)
          cancelModal()
        })
        .catch(() => {
  ...
  useEffect(() => {
    if (params?.levelTwoCategoryId) {
      apiCalls.getCategoriesApi(params.levelTwoCategoryId)
    } else {
      history.goBack()
    }
  }, [])
  ...
  const submitAction = data => {
    if (!isEmpty(selectedCategory)) {
      ...
      const { levelTwoCategoryId } = params
      apiCalls.addCategoryApi({ ...data, parent_id: levelTwoCategoryId })
    }
    cancelModal()
  }
    ...
    <section className="products-section">
      <div className="generic-page-header">
        <div className="header-data-section">
          ...
          <h2 className="page-head my-0">level three categories</h2>
        </div>
        <div className="header-data-section">
